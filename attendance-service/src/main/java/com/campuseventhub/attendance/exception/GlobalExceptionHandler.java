...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<Map<String, Object>> handleNotFound(AttendanceNotFoundException ex, WebRequest req) {
        return buildError(HttpStatus.NOT_FOUND, ex.getMessage(), req);
    }

    @ExceptionHandler(AttendanceAlreadyMarkedException.class)
    public ResponseEntity<Map<String, Object>> handleConflict(AttendanceAlreadyMarkedException ex, WebRequest req) {
        return buildError(HttpStatus.CONFLICT, ex.getMessage(), req);
    }

    @ExceptionHandler(RegistrationServiceUnavailableException.class)
    public ResponseEntity<Map<String, Object>> handleUnavailable(RegistrationServiceUnavailableException ex, WebRequest req) {
        return buildError(HttpStatus.SERVICE_UNAVAILABLE, ex.getMessage(), req);
    }
